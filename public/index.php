...
try {
    ...
    switch (strtoupper(APP_TYPE)) {
        ...
        case 'WEB':
            $kernel = new App(); // crea una instancia del Kernel App
            $response = $kernel->boot();
            break;

        // para Apis
        case 'API':
            $kernel = new Api(); // crea una instancia del Kernel Api
            $response = $kernel->boot();
        ...
        default:
            die('El proyecto solamente puede ser WEB o API.');
    }
    ...
    // prepara el mensaje.
    $mensaje = DEBUG ? 'ERROR en el proceso de arranque: ' . $t : 'Se produjo un error, contacte con el administrador ' . ADMIN_EMAIL . ' si es necesario.';
