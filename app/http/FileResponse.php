...
class FileResponse extends Response
{
     ...
     *
     * @param string $path
     *            ruta al fichero a enviar en la respuesta
     * @param bool $download
     *            indica si hay que descargar o abrir el fichero
     * @param int $httpCode
     *            código HTTP
     * @param string $status
     *            frase correspondiente al estado
     */
    public function __construct(string $path, bool $download = true, int $httpCode = 200, string $status = 'OK')
    {
        $file = new File($path);
    ...
    public function send()
    {
        ...
        echo $this->download ? $this->file->download() : // intenta forzar la descarga
