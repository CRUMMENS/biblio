...
class CSRF
{
    ...
    public static function check(string $token = null, bool $forget = true)
    {
        if (! $token)
    ...
    {
        return Session::has('csrf_token') ? "<input type='hidden' name='csrf_token' value='" . Session::get('csrf_token') . "'>" : "";
    }
