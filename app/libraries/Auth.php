...
class Auth
{
     ...
     *            nombre del rol de administrador, por defecto el que se encuentre configurado en el config.php.
     *            
    ...
    public static function admin(string $adminRole = ADMIN_ROLE)
    {
        ...
        if (! Login::isAdmin($adminRole))
            throw new AuthException(DEBUG ? "Se requiere privilegio de administrador ($adminRole)." : "No estás autorizado a realizar esta operación.");
    }
    ...
    public static function role(string $role)
    {
        ...
        if (! Login::role($role))
            throw new AuthException(DEBUG ? "Se requiere rol de $role para continuar." : "No estás autorizado a realizar esta operación.");
    }
    ...
    public static function allRoles(array $roles)
    {
        ...
        if (! Login::allRoles($roles))
            throw new AuthException(DEBUG ? "Se requieren los roles: " . arrayToString($roles, false, false) . " para continuar." : "No estás autorizado a realizar esta operación.");
    }
    ...
    public static function oneRole(array $roles)
    {
        ...
        if (! Login::oneRole($roles))
            throw new AuthException(DEBUG ? "Se requiere uno de estos roles: " . arrayToString($roles, false, false) . " para continuar." : "No estás autorizado a realizar esta operación.");
    }
}
