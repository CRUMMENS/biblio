...
class DebugInformation
{
     ...
     *            controlador solicitado mediante la URL
     * @param string $metodo
     *            método solicitado mediante la URL
     * @param array $parametros
     *            parámetros adicionales en la URL
     */
    public function __construct(Throwable $t, string $controlador, string $metodo, array $parametros = [])
    ...
    public function toHtml(): string
    {
        ...
        if (in_array('trace', DEBUG_INFO)) {
            ...
            $mensaje .= ! method_exists($this->controller, $this->method) ? "<p>La combinación indicada de controlador y método <b class='error inline-block p1'>NO EXISTE</b>,
                 comprueba si se trata de un error tipográfico o implementa el método
                 <code>$this->method()</code> en el fichero <i>mvc/controllers/$this->controller.php</i>.</p>" : "<p>La combinación indicada de controlador y método <b class='success inline-block p1'>SÍ EXISTE</b>,
        ...
        if (in_array('request', DEBUG_INFO)) {
            ...
            $mensaje .= request()->posts() ? arrayToString(request()->posts(), false, true, '<br>') . "</p>" : "No se recibieron datos.</p>";

            // datos recibidos por GET
            $mensaje .= "<p><span class='label'>GET</span></p><p>";
            $mensaje .= request()->gets() ? arrayToString(request()->gets(), false, true, '<br>') . "</p>" : "No se recibieron datos.</p>";

            // datos recibidos por COOKIE
            $mensaje .= "<p><span class='label'>COOKIE</span></p><p>";
            $mensaje .= request()->cookies() ? arrayToString(request()->cookies(), false, true, '<br>') . "</p>" : "No se recibieron datos.</p>";

            // datos en FILES
            $mensaje .= "<p><span class='label'>FILES</span></p><p>";
            $mensaje .= $_FILES ? arrayToString($_FILES, false, true, '<br>') . "</p>" : "No se recibieron datos.</p>";
