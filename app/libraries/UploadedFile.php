...
class UploadedFile extends File
{
     ...
     *
     * @param string $key
     *            clave de $_FILES a recuperar.
     * @param int $maxSize
     *            tamaño máximo del fichero (0 sin límite)
     * @param array $mimes
     *            lista de tipos MIME permitidos. Lista vacía para cualquier tipo de fichero.
     *            
     * @throws UploadException en caso de que no exista la clave.
     */
    public function __construct(string $key = 'file', int $maxSize = UPLOAD_MAX_SIZE, // definido en config.php
    array $mimes = [])
    {
        if (empty($_FILES[$key]))
     ...
     *            carpeta de destino.
     * @param string $prefix
     *            prefijo para el nombre del fichero en el servidor, en el caso de generar nombres únicos.
     * @param bool $returnFullRoute
     *            retornar ruta completa o solo el nombre del fichero?
     *            
     ...
     */
    public function store(string $folder = UPLOAD_FOLDER, // definido en config.php
    string $prefix = '', bool $returnFullRoute = false): string
    {
     ...
     *            carpeta de destino.
     * @param string $name
     *            nombre del fichero, NULL mantiene el nombre original.
     * @param bool $returnFullRoute
     *            permite seleccionar si el método debe retornar la ruta entera o solamente el nombre del fichero.
     *            
     * @throws FileException si se puede mover el fichero a su ubicación definitiva.
     *        
     * @return string la ruta completa del fichero en el servidor o solamente el nombre.
     */
    public function storeAs(string $folder = UPLOAD_FOLDER, // definido en config.php
    string|NULL $name = NULL, bool $returnFullRoute = false): string
    {
     ...
     *            añade más entropía al nombre generado.
     */
    public function newName(string $prefix = '', string $extension = '', bool $moreEntropy = true)
    {
