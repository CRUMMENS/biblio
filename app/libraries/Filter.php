...
class Filter
{
     ...
     *            texto a buscar
     * @param string $field
     *            campo en el que realizar la búsqueda
     * @param string $orderField
     *            campo para la ordenación de los resultados
     * @param string $order
     *            sentido de la ordenación: ASC o DESC
     */
    public function __construct(string $text = '%', string $field = 'id', string $orderField = 'id', string $order = 'DESC')
    {
        $this->text = $text;
        $this->field = $field;
     ...
     *
     * @param string $name
     *            nombre del filtro en sesión, para que no se confunda con otros filtros que pudieran haber sido aplicados.
     *            
     * @return ?Filter el filtro guardado en sesión o NULL.
     */
    public static function apply(string $name = 'standardFilter'): ?Filter
    {
        $request = request();
        ...
        if ($request->has(self::$fields['filter'])) {
            ...
            $filtro = new Filter($request->post(self::$fields['text']) ?? '', $request->post(self::$fields['field']), $request->post(self::$fields['orderField']), $request->post(self::$fields['order']));
    ...
    {
        $texto = empty($this->text) ? "Todos los resultados. " : "Datos filtrados por <b>'$this->field'</b> = <b>'$this->text'</b>. ";

        $texto .= "Ordenado por <b>'$this->orderField'</b> ";

        $texto .= $this->order == 'ASC' ? " ascendente. " : " descendente. ";
