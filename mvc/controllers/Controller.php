...
abstract class Controller
{
     ...
     *
     * @param string $model
     *            nombre del modelo del que se pretenden exportar datos.
     * @param string $format
     *            formato de exportación
     * @param bool $download
     *            intenta forzar la descarga del fichero con los resultados de la exportación.
     * @param string $orderField
     *            campo de ordenación para los resultados.
     * @param string $order
     *            orden de los resultados.
     *            
     * @return Response
     */
    protected function exportResponse(?string $model = null, string $format = 'JSON', bool $download = false, string $orderField = 'id', string $order = 'ASC'): Response
    {
        ...
        switch ($format) {
            ...
            case 'JSON':
                $response = new JsonResponse(($model)::orderBy($orderField, $order));
                $extension = 'json';
                break;

            case 'XML':
                $response = new XmlResponse(($model)::orderBy($orderField, $order));
                $extension = 'xml';
                break;

            case 'CSV':
                $response = new CsvResponse(($model)::orderBy($orderField, $order));
                $extension = 'csv';
                break;
            ...
            default:
                $response = new ApiResponse(($model)::orderBy($orderField, $order));
                $extension = 'txt';
        }
