...
abstract class Model
{
     ...
     * En caso de no existir esta propiedad,
     ...
     *
     * @param array $data
     *            lista de propiedades de la entidad a modo de array asociativo.
     * @param
     *            int si se trata de una actualización, id del objeto a actualizar
     *            
     ...
     * @return array lista de todas las entidades.
     */
    public static function get(int $limit = 0, int $offset = 0): array
    {
     ...
     *
     * @param int $id
     *            identificador único de la entidad a recuperar.
     *            
     ...
     *            texto a mostrar en caso de no encontrar la entidad con ese identificador.
     *            
     * @throws NotFoundException si no encuentra la entidad con ese identificador.
     * @return object la entidad recuperada.
     */
    public static function findOrFail(int $id = 0, ?string $message = NULL): object
    {
        if (! $id)
     ...
     *            orden deseado para los resultados.
     * @param string $order
     ...
     */
    public static function orderBy(string $orderField = 'id', string $order = 'ASC', int $limit = 0, int $offset = 0): array
    {
        $table = self::getTable(); // recupera el nombre de la tabla
     ...
     * @param string $order
     *            sentido ascendente o descendente para el orden.
     *            
     * @return array lista de entidades con el filtro aplicado.
     */
    public static function getFiltered(string $field = 'id', string $value = '', string $orderField = 'id', string $order = 'ASC'): array
    {
        $table = self::getTable(); // recupera el nombre de la tabla
     ...
     *
     * @param string $field
     *            campo en el que buscar valores nulos
     * @param string $orderField
     *            campo para ordenar resultados
     * @param string $order
     *            sentido del orden (ASC o DESC)
     *            
     * @return array la lista de entidades con ese campo a NULL
     */
    public static function isNull(string $field, string $orderField = 'id', string $order = 'ASC'): array
    {
        $table = self::getTable(); // recupera el nombre de la tabla
     ...
     *
     * @param string $field
     *            campo en el que buscar valores no nulos
     * @param string $orderField
     *            campo para ordenar resultados
     * @param string $order
     *            sentido del orden (ASC o DESC)
     ...
     */
    public static function isNotNull(string $field, string $orderField = 'id', string $order = 'ASC'): array
    {
        $table = self::getTable(); // recupera el nombre de la tabla
    ...
    /**
     * Recupera entidades a partir de un objeto Filter.
     * Se combina con la paginación
     ...
     * @return array lista de entidades con el filtro aplicado.
     */
    public static function filter(Filter $filtro, int $limit = RESULTS_PER_PAGE, int $offset = 0): array
    {
     ...
     *            array asociativo campo => valor con las condiciones.
     * @param string $orden
     *            orden para los resultados.
     * @param string $sentido
     *            sentido ascendente o descendente.
     *            
     * @return array lista de entidades con los filtros aplicados.
     */
    public static function where(array $condiciones = [], string $orden = 'id', string $sentido = 'ASC'): array
    {
        $tabla = self::getTable(); // recupera el nombre de la tabla
     ...
     * @return array lista de entidades con los filtros aplicados.
     */
    public static function whereExactMatch(array $condiciones = [], string $orden = 'id', string $sentido = 'ASC'): array
    {
        $tabla = self::getTable(); // recupera el nombre de la tabla
    ...
    /**
     * Elimina los últimos registros de una tabla en la BDD.
     * En caso
     * de no indicar el número de registros a borrar, los borra todos: PELIGROSO!.
     *
     * @param int $number
     *            número de errores a borrar.
     * @param string $orderField
     *            orden a considerar.
     * @param string $order
     ...
     */
    public static function clear(?int $numero = null, string $orderField = 'id', string $order = 'DESC'): int
    {
        $tabla = self::getTable(); // recupera el nombre de la tabla
     ...
     *            operación deseada.
     * @param string $campo
     *            campo sobre el que realizar la operación.
     *            
     ...
     *
     * @param array $totales
     *            array asociativo en pares de campo => total a aplicar.
     * @param array $agruparPor
     *            array indexado con los nombres de los campos para el agrupado.
     *            
     * @return array con los resultados a modo de objetos stdClass
     */
    public static function groupBy(array $totales = [
        'id' => 'COUNT'
    ], array $agruparPor = [])
    {
        $tabla = self::getTable(); // recupera el nombre de la tabla
     ...
     *            tipo de entidad (clase) relacionada.
     ...
     */
    public function hasAny(string $related, string $foreignKey = null, string $localKey = 'id'): bool
    {
     ...
     *            nombre de la clave primaria en la tabla del lado 1.
     *            
     * @return object|NULL la entidad relacionada en el lado 1.
     */
    public function belongsTo(string $related, string $foreignKey = null, string $ownerKey = 'id'): ?object
    {
        $tabla = $related::$table ?? strtolower($related) . 's'; // cálculo del nombre de la tabla
     ...
     */
    public function belongsToAny(string $related, string $foreignKey = null, string $ownerKey = 'id'): bool
    {
        $table = $related::$table ?? strtolower($related) . 's'; // cálculo del nombre de la tabla
     ...
     * @param string $foreign1
     *            nombre de la clave foránea relacionada con la entidad actual
     * @param string $foreign2
     *            nombre de la clave foránea relacionada con la entidad externa
     * @param string $owner1
     *            nombre de la clave primaria de la entidad actual
     * @param string $owner2
     *            nombre de la clave primaria de la entidad externa
     *            
     * @return array lista de entidades externas relacionadas
     */
    public function belongsToMany(string $related, string $intermediateTable = NULL, string $foreign1 = NULL, string $foreign2 = NULL, string $owner1 = 'id', string $owner2 = 'id'): array
    {
    ...
    public function __toString(): string
    {
        ...
        foreach ($this as $propiedad => $valor) {
            $texto .= is_array($valor) ? "$propiedad: [ " . implode(', ', $valor) . " ]" : "$propiedad: $valor, ";
        }
